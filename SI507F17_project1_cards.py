...
def play_war_game(testing=False):
    ...
    if p1_score > p2_score:
        ...
        return "Tie", p1_score, p2_score8

def new_game(hand_size = 8):
    d = Deck()
    d.shuffle()
    hand1 = d.deal_hand(hand_size)
    hand2 = d.deal_hand(hand_size)
    score_tally = 0

    for i in range(hand_size):
        card_1 = hand1.pop()
        card_2 = hand2.pop()
        if card_1.suit == card_2.suit or card_1.rank == card_2.rank:
            score_tally += 1
...
def show_song(inp="Winner"):
    # default winner ... but also could be something else if it works
    # correctly, which it does not (put in description and remove this
    # clarity)
